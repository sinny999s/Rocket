...
public class OutlinePipeline {
    ...
    private void ensureInitialized() {
        ...
        this.dataBuffer = MemoryUtil.memAlloc((int)512);
        ByteBuffer dummyData = MemoryUtil.memAlloc((int)4);
        dummyData.putInt(0);
        dummyData.flip();
        this.dummyVertexBuffer = RenderSystem.getDevice().createBuffer(() -> "minecraft:outline_dummy_vertex", 32, dummyData);
        MemoryUtil.memFree((Buffer)dummyData);
        this.initialized = true;
    }

    public void drawOutline(float x, float y, float width, float height, int[] colors, float[] thicknesses, float[] radii, float smoothness) {
        ...
        this.prepareUniformData(x, y, width, height, fixedScreenWidth, fixedScreenHeight, 2.0f, colors, thicknesses, radii, smoothness);
        this.uploadAndDraw(client);
    }

    private void prepareUniformData(float x, float y, float width, float height, float screenWidth, float screenHeight, float guiScale, int[] colors, float[] thicknesses, float[] radii, float smoothness) {
        int i;
    ...
    private void uploadAndDraw(Minecraft client) {
        ...
        if (this.uniformBuffer == null || this.uniformBuffer.size() < (long)size) {
            ...
            this.uniformBuffer = RenderSystem.getDevice().createBuffer(() -> "minecraft:outline_uniform", 136, size);
        }
        ...
        try (RenderPass renderPass = encoder.createRenderPass(() -> "minecraft:outline_pass", client.getMainRenderTarget().getColorTextureView(), OptionalInt.empty(), client.getMainRenderTarget().getDepthTextureView(), OptionalDouble.empty());){
            renderPass.setPipeline(PIPELINE);
            renderPass.setVertexBuffer(0, this.dummyVertexBuffer);
            RenderSystem.bindDefaultUniforms(renderPass);
            renderPass.setUniform("DynamicTransforms", dynamicTransforms);
            renderPass.setUniform("OutlineData", this.uniformBuffer);
            renderPass.draw(0, 6);
        }
    }
